...
use lib\gulp\model\GulpTaskModelConfiguration;
...
class GulpfileController
{
    ...
    private function createDb($guid, $createIfNotExists = false)
    {
        ...
        $dir = Context::get()->basePath('app/db/');
        if (!$dir->isDir()){
        ...
        return new JsonRepository(new GulpTaskModelConfiguration(), $filepath);
    }
     ...
     *         409 conflict on double entry TODO
